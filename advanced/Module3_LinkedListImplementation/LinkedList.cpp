...
{
   headPtr = nullptr;
   tailPtr = nullptr;   // initialize tailPtr to nullptr
   itemCount = 0;
...
bool LinkedList<ItemType>::insert(int newPosition, const ItemType& newEntry)
{
   ...
   if (ableToInsert)
   {
      ...
      // Revised Implementation (inserts item into newPosition):
      if (isEmpty())
      {
         headPtr = newNodePtr;
         tailPtr = newNodePtr;
...
void LinkedList<ItemType>::reverse()
{
   ...
   bool ableToReverse = ((curPtr != nullptr) && (curPtr->getNext() != nullptr));
   if (ableToReverse) 
   {
      while (curPtr != nullptr)
      {
         Node <ItemType>*tempPtr = curPtr->getPrev();
